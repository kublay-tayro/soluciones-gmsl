...
const CartDrawer = () => {
    ...
    // Asegurar que el subtotal es calculado correctamente
    ...
    const handleWhatsappCheckout = () => {
        ...
        cart.forEach(item => {
            message += `- ${item.name} (USD ${(Number(item.price) || 0).toLocaleString()})\n`;
        });
                            ...
                            {cart.map((item, index) => (
                                <div key={`${item.id}-${index}`} className="flex gap-4 p-4 bg-slate-50 rounded-lg border border-slate-100">
                                    ...
                                    <div className="flex-grow">
                                        ...
                                        <p className="text-amber-600 font-bold text-sm">USD {(Number(item.price) || 0).toLocaleString()}</p>
                                    </div>
